...
![jsxgraph-air-vscode.png](../../media/images/jsxgraph-air-vscode.png)
...
- `Node.js` (LTS version recommended) — runs the local bridge server that watches your files; includes `npm` for the one-time dependency installation. Download from [nodejs.org](nodejs.org).
- Visual Studio Code — the editor in which you (or your AI assistant) write `construction.js` and `header.js`. Download from [code.visualstudio.com](code.visualstudio.com).
